          ...
          <div v-if="store.notes.length > 0" class="flex flex-col gap-3">
            ...
            >
              ...
              <template v-if="editingNoteId !== note.id">
                ...
                <p v-if="note.tasks?.title" class="text-xs text-blue-400 mt-2">
                  🔗 {{ note.tasks.title }}
                </p>
              ...
              <template v-else>
                <textarea
                  ...
                  rows="4"
                  class="w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2.5 text-sm text-gray-900 dark:text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                  @keydown.ctrl.enter="submitEditNote(note.id)"
                  @keydown.meta.enter="submitEditNote(note.id)"
                />
                <div class="flex justify-end gap-2 mt-3 pt-3 border-t border-gray-100 dark:border-gray-800">
                  <button
                    type="button"
            ...
            <div class="flex-1 min-h-0 overflow-y-auto px-6 flex flex-col gap-4 pb-2">
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <div class="flex flex-wrap gap-1.5">
                  ...
                  <span v-if="store.allUserTags.length === 0" class="text-xs text-gray-400">
...
const activeFilterCount = computed(() =>
    mobilePriority.value.length + mobileStatus.value.length + mobileTag.value.length
)
...
const taskForm  = ref({ title: '', priority: '', status: '', due_date: '', description: '', note: '', noteId: null, tags: [] })

function openTaskModal(task = null) {
  taskModal.value = { open: true, task }
  const latestNote = task ? store.notes.find(n => n.task_id === task.id) ?? null : null
  taskForm.value = task
      ? { title: task.title, priority: task.priority, status: task.status, due_date: task.due_date ?? '', description: task.description ?? '', note: latestNote?.content ?? '', noteId: latestNote?.id ?? null, tags: (task.task_tags ?? []).map(tt => tt.tags) }
      : { title: '', priority: '', status: '', due_date: '', description: '', note: '', noteId: null, tags: [] }
}

const availableModalTags = computed(() =>
    store.allUserTags.filter(t => !taskForm.value.tags.find(st => st.id === t.id))
)
    ...
    if (taskModal.value.task) {
      ...
      if (noteContent) {
        if (taskForm.value.noteId) {
          await store.updateNote(taskForm.value.noteId, noteContent)
        } else {
          await store.addNote(noteContent, taskModal.value.task.id)
        }
      }
      // sync tags
      ...
      if (noteContent && newTask) {
        await store.addNote(noteContent, newTask.id)
      }
...
async function submitEditNote(noteId) {
  ...
  await store.updateNote(noteId, editingNoteContent.value.trim())
  cancelEditNote()
}
